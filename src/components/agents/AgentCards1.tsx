...
interface AgentActivity {
  id?: string;
  agentId: string;
  activityType: 'login' | 'logout' | 'lead_assigned' | 'lead_updated' | 'call' | 'email' | 'whatsapp' | 'edit' | 'status_change';
...
export const AgentCards: React.FC = () => {
    ...
    const fetchActivities = async () => {
      try {
        ...
        const unsubscribe = onValue(activitiesRef, (snapshot) => {
          ...
          const activitiesArray: AgentActivity[] = [];

          Object.entries(activitiesData).forEach(([id, activityData]: [string, any]) => {
            try {
              // Check if activityData is already decrypted or needs processing
              const activity: AgentActivity = {
                ...
                ...(activityData.leadId && { leadId: activityData.leadId }),
                ...(activityData.metadata && { metadata: activityData.metadata })
              };

              // Only add if it belongs to the current agent
              if (activity.agentId === viewingAgent.id) {
          ...
          setAgentActivities(sortedActivities);
          setLoadingActivities(false);
        });
  ...
  const formatActivityDetails = (activity: AgentActivity) => {
    if (typeof activity.activityDetails === 'string') {
      try {
        const parsedDetails = JSON.parse(activity.activityDetails);
        return (
        ...
        return <p className="text-sm mt-1">{activity.activityDetails}</p>;
      }
    }
    return (
            ...
            {currentAgents.map((agent) => (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${agent.status === 'active'
                    ...
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                      ...
                      <div className="text-center sm:text-left">
                        ...
                        <div className="flex items-center justify-center sm:justify-start mt-2">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${viewingAgent.status === 'active'
                              ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300'
                              : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300'
                            }`}>
                        ...
                        {agentActivities.map((activity) => (
                          <div key={activity.id} className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                            <div className="flex-shrink-0 mt-1">
                              ...
                              {activity.activityType === 'edit' && <Edit className="h-5 w-5 text-orange-500" />}
                              {activity.activityType === 'status_change' && <Activity className="h-5 w-5 text-indigo-500" />}
                            </div>
