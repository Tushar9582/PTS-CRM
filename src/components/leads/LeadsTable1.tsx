...
export const LeadsTable: React.FC = () => {
  ...
  const updateLeadScores = async () => {
    ...
    try {
      ...
      leads.forEach(lead => {
        ...
        // Remove this line: updates[`${lead.id}/updatedAt`] = new Date().toISOString();
      });
  ...
  try {
    // Fetch existing lead data from Firebase to compare
    const leadSnapshot = await get(ref(database, `users/${adminId}/leads/${updatedLead.id}`));
    const existingLead = leadSnapshot.val();

    // Determine which fields have been updated
    const updatedFields: { [key: string]: any } = {};
    for (const key in updatedLead) {
      if (updatedLead[key] !== existingLead[key]) {
        updatedFields[key] = updatedLead[key];
      }
    }

    // Update the lead
    await update(ref(database, `users/${adminId}/leads/${updatedLead.id}`), updatedFields);

    // Create an activity log entry
    const activity = {
      action: 'update',
      leadId: updatedLead.id,
      updatedFields: updatedFields, // only the changed fields!
      agentId: localStorage.getItem('agentId'),
      timestamp: new Date().toISOString(),
    };

    // Push the activity log to agentActivity
          ...
          <tbody className="divide-y divide-border">
            {currentLeads.map((lead) => (
              ...
              >
                ...
                <td className="p-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${lead.Meeting_Status === 'new' ? 'bg-blue-100 text-blue-800' :
                ...
                <td className="p-3">
                  <div className="flex items-center">
                    ...
                    <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div
                        className={`h-full ${(lead.score || 0) >= 80 ? 'bg-green-500' :
